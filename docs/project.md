...
- Automated tests use GUT (`addons/gut/`). Test scripts extend `GutTest`, live under `test/unit/<category>/` mirroring `components/<category>/`, and use a `*_test.gd` filename suffix (not GUT's default `test_*` prefix), configured via `.gutconfig.json` at the project root.
